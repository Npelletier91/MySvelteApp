<script>
    ...
    export let monthBackground; // Expecting this prop from the parent
...
$: time = `${apptDetails.hour}:${apptDetails.minutes < 10 ? '0'+apptDetails.minutes : apptDetails.minutes}${apptDetails.amOrPM}`;
	...
	const submitAppt = () => {
		console.log("Submitting appointment:", apptDetails);
...
<section transition:fade={{ duration: 125 }}>
	<form method="post"
		...
		<header style="background-color: {monthBackground}">
